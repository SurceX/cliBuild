...
2. Create a package with your name and move `com.cesur.Main.java` inside it:

```
src/com/<yourname>/com.cesur.Main.java
```

For example, if your name is Franco: `src/com/franco/com.cesur.Main.java`

The starter `com.cesur.Main.java` contains TODOs to guide the first steps:
...
graph TB
    subgraph Presentation
        M[com.cesur.Main.java<br>CLI Menu Loop]
    end
...
- Implement the CLI menu loop in `com.cesur.Main.java`
...
**Wiring in com.cesur.Main.java** — the student can now swap between array and file implementations for bookings:
...
These can be swapped in via dependency injection in `com.cesur.Main.java`:
...
Place this bean in the `com.cesur.Main` class (annotated with `@SpringBootApplication`). The data is inserted into H2 on every startup since the database is in-memory.
...
│   │   └── com/
│   │       └── <yourname>/
│   │           ├── com.cesur.Main.java                       (@SpringBootApplication)
│   │           ├── car/
│   │           │   ├── Brand.java
│   │           │   ├── Car.java                    (@Entity)
